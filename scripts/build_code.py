...
def build(code,test,rootdir)
 wrf_tar_name = rootdir + "tarballs/"
 wrf_parallel = list()                # Will hold builds that can be built in parallel (independently, all building at the same time)
 wrf_serial = list()                  # Will hold builds that must be built consecutively (usually relying on pre-built code, such as em_real is built first, then em_quarter_ss).
 if test.batch:
 else:
    for bt in test.build_types:
       if bt.depend:              # If this build type has dependancies, set status to 'waiting', we'll return later
          bt.status = 'waiting'
          continue
       else:
          #submit build!
